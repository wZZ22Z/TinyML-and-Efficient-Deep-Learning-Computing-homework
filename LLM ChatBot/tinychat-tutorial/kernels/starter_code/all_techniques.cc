    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i lower_half = _mm256_and_si256(raw_w,lowMask);
                __m256i upper_half = _mm256_and_si256(_mm256_srli_epi16(raw_w,4),lowMask);
                __m256i raw_w_next = _mm256_loadu_si256(w_start + 1);
                __m256i lower_half_next = _mm256_and_si256(raw_w_next,lowMask);
                __m256i upper_half_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next,4),lowMask);
                ...
                w_0_next=_mm256_sub_epi8(lower_half_next,zero_point);
                w_128_next = _mm256_sub_epi8(upper_half_next,zero_point);
